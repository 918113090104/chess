    ...
    ui(new Ui::MainWindow)
{
   ...
   time->setFixedSize(160,100);
   time->setPalette(Qt::red);
   time->setGeometry(680,150,160,100);
   timer=new QTimer(this);
   timer->setInterval(1000);
   button=new QPushButton("开始游戏",this);
   button->setGeometry(690,445,140,70);
...
void MainWindow::slotStart(){
    ...
    button->setText("重新开始");
    m_time=500;
    time->display(QString::number(m_time));
    timer->start();
    update();
...
{
    if(r_col==1){
       QMessageBox::about(this,"游戏结果","恭喜黑方获胜！祝红方再接再厉");
       r_col=0;
       b_col=0;
    ...
    if(b_col==16){
       QMessageBox::about(this,"游戏结果","恭喜红方获胜！祝黑方再接再厉");
       r_col=0;
       b_col=0;
       return true;
    }
...
void MainWindow::Timeout(){
    ...
    if (m_time == 0)
        {
            timer->stop();
            QMessageBox::about(this,"游戏结果","时间到！本局和棋");
            return;
        }
